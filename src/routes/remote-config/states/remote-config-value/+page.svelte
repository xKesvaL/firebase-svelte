<script>
	...
	const title = `Remote Config Value Store | Firebase Svelte`;
	const description = `createRemoteConfigValueStore(remoteConfig, key, options) creates a store that will activate the current remoteconfig instance. The store contains a value retrieved from the remote config.`;
	const url = `https://firebase-svelte.vercel.app/remote-config/stores/remote-config-value`;
	const image = ``;
</script>
...
<section>
	<div class="container flex flex-col gap-8 py-8 lg:py-16 xl:py-24">
		<h1 id="create-remote-config-value-store" class="font-mono text-4xl font-black lg:text-5xl">
			createRemoteConfigValueStore()
		</h1>
		<p class="max-w-prose">
			<code>createRemoteConfigValueStore&lt;T&gt;(remoteConfig, key, options)</code> creates a store
		...
		<ul class="flex max-w-prose flex-col gap-4">
			...
			<li>
				<code>options?: RemoteConfigStoreOptions&lt;T&gt;</code>
			</li>
		</ul>
