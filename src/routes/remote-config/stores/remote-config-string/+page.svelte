<script>
	...
	const title = `Remote Config String Store | Firebase Svelte`;
	const description = `createRemoteConfigStringStore(remoteConfig, key, options) creates a store that will activate the current remoteconfig instance. The store contains a string value retrieved from the remote config.`;
	const url = `https://firebase-svelte.vercel.app/remote-config/stores/remote-config-string`;
	const image = ``;
</script>
...
<section>
	<div class="container flex flex-col gap-8 py-8 lg:py-16 xl:py-24">
		<h1 id="create-remote-config-string-store" class="font-mono text-4xl font-black lg:text-5xl">
			createRemoteConfigStringStore()
		</h1>
		<p class="max-w-prose">
			<code>createRemoteConfigStringStore(remoteConfig, key, options)</code> creates a store that will
			activate the current remoteconfig instance. The store contains a string value retrieved from the
			remote config.
		</p>
		...
		<ul class="flex max-w-prose flex-col gap-4">
			...
			<li>
				<code>key: string</code> - The remote config key.
			</li>
			<li>
				<code>options?: RemoteConfigStoreOptions&lt;string&gt;</code>
			</li>
		</ul>
		<h2 id="properties" class="text-3xl lg:text-4xl">Properties</h2>
		<ul class="flex max-w-prose flex-col gap-4">
			<li>
				<code>subscribe: ((val: string) => void)) => void</code> - Subscribe method for the Svelte store.
				Allows for the `$store` syntax.
			</li>
			<li>
				<code>loading: boolean</code> - When the remote config key is loading.
			</li>
			<li>
