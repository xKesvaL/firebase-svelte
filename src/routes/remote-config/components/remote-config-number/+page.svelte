<script>
	...
	const title = `Remote Config Number Component | Firebase Svelte`;
	const description = `The <RemoteConfigNumber> component is a component that renders different content depending on the data in a remote config number value and gives you access to the remote config instance.`;
	const url = `https://firebase-svelte.vercel.app/remote-config/components/remote-config-number`;
	const image = ``;
</script>
...
<section>
	<div class="container flex flex-col gap-8 py-8 lg:py-16 xl:py-24">
		<h1 id="remote-config-number" class="font-mono text-4xl font-black lg:text-5xl">
			&lt;RemoteConfigNumber&gt;
		</h1>
		...
		<ul class="flex max-w-prose flex-col gap-4">
			...
			<li>
				<code>key: string</code> - The key of the remote config number value.
			</li>
		</ul>
		<h2 id="props" class="text-3xl xl:text-4xl">Slots & Props</h2>
		<div class="flex flex-col gap-4">
			...
			<ul class="flex max-w-prose flex-col gap-2">
				<li>
					<code>value: number</code> - The remote config number value.
				</li>
			</ul>
		</div>
		<div class="flex flex-col gap-4">
