<script>
	...
	const title = `Node List Component | Firebase Svelte`;
	const description = `The <NodeList> component is a component that renders different content depending on the data in a node and gives you access to the node reference.`;
	const url = `https://firebase-svelte.vercel.app/realtime-db/components/node-list`;
	const image = ``;
</script>
...
<section>
	<div class="container flex flex-col gap-8 py-8 lg:py-16 xl:py-24">
		<h1 id="node-list" class="font-mono text-4xl font-black lg:text-5xl">&lt;NodeList&gt;</h1>
		<h2 id="props" class="text-3xl xl:text-4xl">Props</h2>
		<ul class="flex max-w-prose flex-col gap-4">
			<li>
				<code>rtdb?: Database</code> - The realtime database instance, optional if you put the <Button
					variant="link"
		...
		<div class="flex flex-col gap-4">
			...
			<ul class="flex max-w-prose flex-col gap-2">
				...
				<li>
					<code>count: number</code> - The number of items in the list.
				</li>
			</ul>
		</div>
		<div class="flex flex-col gap-4">
